...
// this is modified from git-core gitmkstemps()
#include <sys/stat.h>
#include <errno.h>
#ifdef __MORPHOS__
...
char *mktemps(char *pattern, size_t suffix_len)
{
	...
	tmplt = &pattern[len - 6 - suffix_len];
	for (count = 0; count < TMP_MAX; ++count) {
		uint64_t v = value;
		/* Fill in the random bits. */
		tmplt[0] = letters[v % num_letters]; v /= num_letters;
		tmplt[1] = letters[v % num_letters]; v /= num_letters;
		tmplt[2] = letters[v % num_letters]; v /= num_letters;
		tmplt[3] = letters[v % num_letters]; v /= num_letters;
		tmplt[4] = letters[v % num_letters]; v /= num_letters;
		tmplt[5] = letters[v % num_letters]; v /= num_letters;

		if (stat(pattern, &st) == -1) {
			if (errno == ENOENT)
				return pattern;	/* it doesn't exist */
			else
				break;	/* fatal error */
		} else {
			if (!S_ISREG(st.st_mode))
				return pattern; /* it's not a regular file */
		}
...
#if defined(__AROS__) || defined(__MORPHOS__) || defined(__amigaos4__)
	// mktemp cannot guarantee a unique filename, if we modify the template after the call.
	// At least not on AROS, so here's one which can cope with a suffix.
	strcpy(filename,"profileXXXXXX.ini");
	mktemps(filename, 4);
	//bprintf("Profile created for %s: %s\n", p_name, filename);
#else
